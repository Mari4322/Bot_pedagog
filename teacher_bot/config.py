...
def load_config() -> Config:
    ...
    # Важно: `.env` лежит рядом с кодом, поэтому load_dotenv() его подхватит.
    load_dotenv()

    # Читаем переменные окружения. strip() убирает лишние пробелы.
    ...
    # TZ нужен для APScheduler, чтобы “00:00” было вашим реальным “00:00” (UTC+7).
    timezone = os.getenv("TZ", "Asia/Novosibirsk").strip()

    # Валидируем обязательные настройки — иначе бот не сможет работать.
    if not bot_token:
